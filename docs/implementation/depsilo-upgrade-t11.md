...
- 新增 `scripts/benchmark-install.sh`，固定 Python `requests==2.32.3` 与 Node.js `lodash@4.17.21`，记录 direct cold、Depsilo client cold、Depsilo hot 和 warm client 直连/代理五组原始样本。
- 每个场景使用独立临时客户端缓存；预热请求不计入计时；结果保留每次状态码、wall time、中位数、范围和失败数，不计算或宣称加速/节省比例。
