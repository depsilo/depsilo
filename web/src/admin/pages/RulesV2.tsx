...
export default function RulesV2() {
      ...
      <div className="space-y-6">
        <div className="text-center py-12 rounded-[8px]" style={{ background: 'rgba(83,58,253,0.04)', border: '1px solid var(--border-purple)' }}>
          <div className="flex flex-col items-center gap-4">
            <div className="flex items-center justify-center w-14 h-14 rounded-[8px]" style={{ background: 'rgba(83,58,253,0.08)' }}><Icon name="shield" size="lg" style={{ color: 'var(--stripe-purple)' }} /></div>
            <h3 className="text-[18px] font-[300]" style={{ color: 'var(--heading)' }}>{t('rules.proRequired')}</h3>
            <p className="text-[14px] max-w-md" style={{ color: 'var(--body)' }}>{t('rules.proDesc')}</p>
  ...
  const columns = [
    ...
    { key: 'package_name', label: t('rules.packageName'), render: (v: unknown) => <span className="font-mono text-[12px]" style={{ color: 'var(--heading)' }}>{v as string}</span> },
    { key: 'version', label: t('rules.version'), render: (v: unknown) => <span className="font-mono text-[12px]" style={{ color: 'var(--body)' }}>{(v as string) === '*' ? t('rules.allVersions') : (v as string)}</span> },
    ...
    { key: 'reason', label: t('rules.reason'), render: (v: unknown) => <span className="text-[12px] truncate block max-w-[200px]" style={{ color: 'var(--body)' }} title={v as string}>{(v as string) || '—'}</span> },
    { key: 'created_at', label: t('users.createdAt'), render: (v: unknown) => <span className="text-[12px] whitespace-nowrap" style={{ color: 'var(--body)' }}>{formatTime(v as string)}</span> },
    { key: 'id', label: t('actions'), render: (_v: unknown, row: any) => (<div className="flex gap-1"><button onClick={() => openEdit(row)} className="bg-transparent cursor-pointer p-1.5 rounded-[4px]" style={{ color: 'var(--body)' }}><Icon name="edit" size="sm" /></button><button onClick={() => setDeleteTarget(row.id)} className="bg-transparent cursor-pointer p-1.5 rounded-[4px]" style={{ color: 'var(--body)' }}><Icon name="delete" size="sm" /></button></div>) },
  ]
  ...
  return (
    <div className="space-y-6">
      ...
      <CardV2 noPad>{isLoading ? <div className="p-8 text-center text-[14px]" style={{ color: 'var(--body)' }}>{t('loading')}</div> : items.length === 0 ? <div className="p-8 text-center text-[14px]" style={{ color: 'var(--body)' }}>{t('rules.noRules')}</div> : <DataTableV2 columns={columns} data={items} />}</CardV2>
      ...
      <ModalV2 open={dialogOpen} onClose={closeDialog} title={editId ? t('rules.editRule') : t('rules.addRule')}>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-[14px] font-[400] mb-1" style={{ color: 'var(--label)' }}>{t('rules.action')}</label>
            <div className="flex gap-2">
              <button type="button" onClick={() => setForm({ ...form, action: 'allow' })} className="flex-1 py-2 text-[14px] font-[400] rounded-[4px] transition-colors cursor-pointer" style={{ background: form.action === 'allow' ? 'rgba(21,190,83,0.15)' : 'var(--surface-low)', color: form.action === 'allow' ? 'var(--success-text)' : 'var(--body)', border: form.action === 'allow' ? '1px solid rgba(21,190,83,0.4)' : '1px solid var(--border)' }}>{t('rules.allow')}</button>
              <button type="button" onClick={() => setForm({ ...form, action: 'deny' })} className="flex-1 py-2 text-[14px] font-[400] rounded-[4px] transition-colors cursor-pointer" style={{ background: form.action === 'deny' ? 'var(--error-container)' : 'var(--surface-low)', color: form.action === 'deny' ? 'var(--error)' : 'var(--body)', border: form.action === 'deny' ? '1px solid var(--error)' : '1px solid var(--border)' }}>{t('rules.deny')}</button>
            </div>
          </div>
          <div><label className="block text-[14px] font-[400] mb-1" style={{ color: 'var(--label)' }}>{t('rules.reason')}</label><textarea value={form.reason} onChange={(e) => setForm({ ...form, reason: e.target.value })} placeholder={t('rules.reasonPlaceholder')} rows={2} className="w-full rounded-[4px] px-3 py-2.5 text-[16px] resize-none" style={{ background: 'var(--surface)', border: '1px solid var(--border)', color: 'var(--heading)', outline: 'none' }} /></div>
      ...
      <ModalV2 open={deleteTarget !== null} onClose={() => setDeleteTarget(null)} title={t('rules.confirmDelete')}>
        <p className="text-[14px] mb-6" style={{ color: 'var(--body)' }}>{t('rules.confirmDeleteMsg')}</p>
      ...
      <ModalV2 open={testOpen} onClose={() => setTestOpen(false)} title={t('rules.testTitle')}>
        <div className="space-y-4">
          ...
          {testResult && !testResult.error && (
            <div className="rounded-[4px] p-4" style={{ background: testResult.allowed ? 'rgba(21,190,83,0.1)' : 'var(--error-container)', border: `1px solid ${testResult.allowed ? 'rgba(21,190,83,0.3)' : 'var(--error)'}` }}>
              <div className="flex items-center gap-2 mb-2"><Icon name={testResult.allowed ? 'check_circle' : 'cancel'} size="sm" style={{ color: testResult.allowed ? 'var(--success)' : 'var(--error)' }} /><span className="font-[400] text-[14px]" style={{ color: testResult.allowed ? 'var(--success-text)' : 'var(--error)' }}>{testResult.allowed ? t('rules.resultAllowed') : t('rules.resultDenied')}</span></div>
              {testResult.matched_rule ? (<div className="text-[12px] space-y-1" style={{ color: 'var(--body)' }}><p className="font-[400]" style={{ color: 'var(--heading)' }}>{t('rules.matchedRule')}:</p><p className="font-mono">{testResult.matched_rule.ecosystem}/{testResult.matched_rule.package_name}@{testResult.matched_rule.version} → {testResult.matched_rule.action}</p>{testResult.matched_rule.reason && <p>{testResult.matched_rule.reason}</p>}</div>) : (<p className="text-[12px]" style={{ color: 'var(--body)' }}>{t('rules.noMatch')}</p>)}
            </div>
          )}
          {testResult?.error && <div className="rounded-[4px] p-4" style={{ background: 'var(--error-container)', border: '1px solid var(--error)' }}><p className="text-[14px]" style={{ color: 'var(--error)' }}>{testResult.error}</p></div>}
        </div>
      </ModalV2>
    </div>
  )
