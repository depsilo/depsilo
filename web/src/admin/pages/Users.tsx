...
export default function UsersV2() {
  ...
  const userColumns = [
    { key: 'username', label: t('users.user'), render: (_v: unknown, row: AdminUser & Record<string, unknown>) => (<div className="flex items-center gap-3"><div className="flex h-8 w-8 items-center justify-center rounded-sm text-body font-medium shrink-0 bg-primary text-primary-foreground">{row.username?.[0]?.toUpperCase() || '?'}</div><span className="font-medium text-foreground">{row.username}</span></div>) },
