...
export default function UsersV2() {
  ...
  const userColumns = [
    { key: 'username', label: t('users.user'), render: (_v: unknown, row: AdminUser & Record<string, unknown>) => (<div className="flex items-center gap-3"><div className="flex h-8 w-8 items-center justify-center rounded-[6px] text-body font-medium shrink-0 bg-primary text-primary-foreground">{row.username?.[0]?.toUpperCase() || '?'}</div><span className="font-medium text-foreground">{row.username}</span></div>) },
  ...
  const tokenColumns = [
    { key: 'name', label: t('name'), render: (v: unknown) => <span className="font-medium text-foreground">{v as string}</span> },
    { key: 'permissions', label: t('users.permissions'), render: (v: unknown) => <BadgeV2>{v as string}</BadgeV2> },
    { key: 'last_used_at', label: t('users.lastUsed'), render: (v: unknown) => <span className="font-mono text-label text-muted-foreground">{formatTime(v as string)}</span> },
    { key: 'expires_at', label: t('users.expiresAt'), render: (v: unknown) => <span className="font-mono text-label text-muted-foreground">{v ? formatTime(v as string) : t('users.neverExpires')}</span> },
    { key: 'id', label: t('actions'), render: (_v: unknown, row: APITokenSummary & Record<string, unknown>) => canWrite ? <ButtonV2 variant="ghost" size="sm" className="!text-label text-destructive" onClick={(e: React.MouseEvent) => { e.stopPropagation(); openRevokeDialog(row) }}>{t('users.revoke')}</ButtonV2> : null },
  ]
  ...
  return (
    ...
    <div className="space-y-12">
      ...
      <ModalV2 open={tokenResultOpen} onClose={() => setTokenResultOpen(false)} title={t('users.tokenGenerated')}>
        <p className="text-body mb-3 text-muted-foreground">{t('users.tokenCopyWarning')}</p>
        <div className="flex items-center gap-2 rounded-[4px] border border-border bg-muted p-3">
          <code className="flex-1 font-mono text-body break-all text-foreground">{createdToken}</code>
          <IconButton icon={copied ? Check : Copy} label={t('users.copyToken')} onClick={copyToken} />
        </div>
        <p className="text-label mt-2 text-destructive">{t('users.tokenSaveWarning')}</p>
