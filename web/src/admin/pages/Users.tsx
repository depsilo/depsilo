...
export default function UsersV2() {
  ...
  const userColumns = [
    ...
    { key: 'last_login_at', label: t('users.lastLogin'), render: (v: unknown) => <span className="font-mono text-[12px]" style={{ color: 'var(--body)' }}>{formatTime(v as string)}</span> },
    { key: 'created_at', label: t('users.createdAt'), render: (v: unknown) => <span className="font-mono text-[12px]" style={{ color: 'var(--body)' }}>{formatTime(v as string)}</span> },
    { key: 'id', label: t('actions'), render: (_v: unknown, row: any) => (<div className="flex gap-1"><button onClick={(e) => { e.stopPropagation(); openEditUser(row) }} className="bg-transparent cursor-pointer p-1.5 rounded-[4px]" style={{ color: 'var(--body)' }}><Icon name="edit" size="sm" /></button><button onClick={(e) => { e.stopPropagation(); toggleUserMutation.mutate({ id: row.id, enabled: !row.enabled }) }} className="bg-transparent cursor-pointer p-1.5 rounded-[4px]" style={{ color: 'var(--body)' }}><Icon name={row.enabled ? 'person_off' : 'person'} size="sm" /></button></div>) },
  ]

  const tokenColumns = [
    { key: 'name', label: t('name'), render: (v: unknown) => <span className="font-[400]" style={{ color: 'var(--heading)' }}>{v as string}</span> },
    { key: 'permissions', label: t('users.permissions'), render: (v: unknown) => <BadgeV2>{v as string}</BadgeV2> },
    { key: 'last_used_at', label: t('users.lastUsed'), render: (v: unknown) => <span className="font-mono text-[12px]" style={{ color: 'var(--body)' }}>{formatTime(v as string)}</span> },
    { key: 'expires_at', label: t('users.expiresAt'), render: (v: unknown) => <span className="font-mono text-[12px]" style={{ color: 'var(--body)' }}>{v ? formatTime(v as string) : t('users.neverExpires')}</span> },
    { key: 'id', label: t('actions'), render: (v: unknown) => <ButtonV2 variant="ghost" size="sm" className="!text-[12px]" style={{ color: 'var(--error)' }} disabled={deleteTokenMutation.isPending} onClick={(e: React.MouseEvent) => { e.stopPropagation(); deleteTokenMutation.mutate(v as number) }}>{t('users.revoke')}</ButtonV2> },
  ]
  ...
  return (
    <div className="space-y-6">
      ...
      <CardV2 noPad>{usersLoading ? <div className="p-8 text-center text-[14px]" style={{ color: 'var(--body)' }}>{t('loading')}</div> : users.length === 0 ? <div className="p-8 text-center text-[14px]" style={{ color: 'var(--body)' }}>{t('users.noUsers')}</div> : <DataTableV2 columns={userColumns} data={users} />}</CardV2>

      <CardV2>
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-[12px] uppercase tracking-wider font-[400]" style={{ color: 'var(--body)' }}>API TOKENS</h3>
        ...
        <div className="-mx-5 -mb-5 overflow-hidden rounded-b-[5px]">
          {tokensLoading ? <div className="p-8 text-center text-[14px]" style={{ color: 'var(--body)' }}>{t('loading')}</div> : tokens.length === 0 ? <div className="p-8 text-center text-[14px]" style={{ color: 'var(--body)' }}>{t('users.noTokens')}</div> : <DataTableV2 columns={tokenColumns} data={tokens} />}
        </div>
      </CardV2>
      ...
      <ModalV2 open={tokenResultOpen} onClose={() => setTokenResultOpen(false)} title={t('users.tokenGenerated')}>
        <p className="text-[14px] mb-3" style={{ color: 'var(--body)' }}>{t('users.tokenCopyWarning')}</p>
        <div className="flex items-center gap-2 rounded-[4px] p-3" style={{ background: 'var(--surface-low)', border: '1px solid var(--border)' }}>
          <code className="flex-1 font-mono text-[13px] break-all" style={{ color: 'var(--heading)' }}>{createdToken}</code>
          <button onClick={copyToken} className="bg-transparent cursor-pointer p-1.5 shrink-0 rounded-[4px]" style={{ color: 'var(--body)' }}><Icon name={copied ? 'check' : 'content_copy'} size="sm" /></button>
        </div>
        <p className="text-[12px] mt-2" style={{ color: 'var(--error)' }}>{t('users.tokenSaveWarning')}</p>
