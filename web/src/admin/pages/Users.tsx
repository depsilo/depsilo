...
export default function UsersV2() {
  ...
  const userColumns = [
    ...
    { key: 'id', label: t('actions'), render: (_v: unknown, row: any) => (<div className="flex gap-1"><button onClick={(e) => { e.stopPropagation(); openEditUser(row) }} className="bg-transparent cursor-pointer p-1.5 rounded-[4px] transition-[color,background,transform] duration-150 active:scale-[0.96] hover:text-[var(--text)]" style={{ color: 'var(--text-soft)' }}><Icon name="edit" size="sm" /></button><button onClick={(e) => { e.stopPropagation(); toggleUserMutation.mutate({ id: row.id, enabled: !row.enabled }) }} className="bg-transparent cursor-pointer p-1.5 rounded-[4px] transition-[color,background,transform] duration-150 active:scale-[0.96] hover:text-[var(--text)]" style={{ color: 'var(--text-soft)' }}><Icon name={row.enabled ? 'person_off' : 'person'} size="sm" /></button></div>) },
  ]
