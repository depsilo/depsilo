...
function ChannelButton({
  ...
  return (
    ...
    >
      ...
      <code className="mt-0.5 font-mono text-[10px] leading-[1.3] text-muted-foreground">
        {choice.channel}
      </code>
    </button>
  )
    ...
    >
      <details className="config-disclosure">
        <summary className="stripe-focus-ring flex min-h-[68px] cursor-pointer list-none items-center gap-3 rounded-[6px] py-3">
          ...
          <span className="min-w-0 flex-1">
            <span className="flex flex-wrap items-center gap-2">
              <span
                id={titleId}
                className="text-[13px] font-[650] leading-[1.35] text-foreground"
              >
                {t('quickstart.pytorchIndexTitle')}
              </span>
              <Badge variant="neutral">{t('quickstart.pytorchIndexCache')}</Badge>
            </span>
            <span className="mt-1 block max-w-[72ch] text-[12px] leading-[1.45] text-muted-foreground">
              {t('quickstart.pytorchIndexSummary')}
            </span>
          </span>
          <span className="flex shrink-0 items-center gap-1.5 text-[12px] font-[600] text-primary">
        ...
        <div className="pb-4 sm:pl-11">
          <p className="mb-3 mt-0 max-w-[72ch] text-[12px] leading-[1.5] text-muted-foreground">
            {t('quickstart.pytorchIndexDescription')}
          </p>
          <fieldset className="m-0 min-w-0 border-0 p-0">
            <legend className="mb-2 text-[12px] font-[650] leading-[1.3] text-muted-foreground">
            ...
            <p className="mb-0 mt-2 text-[12px] leading-[1.45] text-muted-foreground">
          ...
          {channelIsValid && (
            <>
              ...
              >
                <span className="text-[11px] font-[620] text-muted-foreground">
                  {t('quickstart.pytorchIndexEndpointLabel')}
                </span>
                <code className="break-all font-mono text-[11px] leading-[1.45] text-primary">
                  {indexURL}
                </code>
              </div>
              <CodeBlock
