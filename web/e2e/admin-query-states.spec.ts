...
const recoveryQueries: readonly QueryCase[] = [
  {
    ...
    success: adminApiDefaults["GET /api/v1/admin/dashboard"],
  },
  {
    path: "/admin/cache",
    endpoint: "GET /api/v1/admin/cache",
    success: adminApiDefaults["GET /api/v1/admin/cache"],
    siblings: { "GET /api/v1/admin/cache/distribution": populatedDistribution },
  },
  {
    path: "/admin/users",
    endpoint: "GET /api/v1/admin/users",
    success: adminApiDefaults["GET /api/v1/admin/users"],
    siblings: { "GET /api/v1/admin/tokens": [populatedToken] },
  },
];
...
const permissionQueries: readonly QueryCase[] = [
  {
    ...
    success: adminApiDefaults["GET /api/v1/admin/security/dashboard"],
  },
];

async function expectPrimarySiblingVisible(page: Page, path: string) {
  if (path === "/admin/users")
    await expect(page.getByText("fixture-ci")).toBeVisible();
  if (path === "/admin/cache")
    await expect(page.getByText("requests")).toBeVisible();
}

for (const query of recoveryQueries) {
  test(`${query.path} recovers from an initial 500 only after manual Retry`, async ({
    page,
  }) => {
    let calls = 0;
    await mockAdminApi(page, {
      ...
      [query.endpoint]: () => {
        ...
        return calls === 1
          ? {
              status: 500,
              body: { code: "FAILED", message: "fixture initial failure" },
            }
          : query.success;
      },
    });
    ...
    const error = page
      .getByRole("alert")
      .filter({ hasText: "fixture initial failure" });
...
for (const query of permissionQueries) {
  test(`${query.path} renders 403 as permission denied rather than empty`, async ({
    page,
  }) => {
    await mockAdminApi(page, {
      ...query.siblings,
...
test("Projects 402 renders the Pro upgrade callout rather than empty or permission denied", async ({
  page,
}) => {
  await mockAdminApi(page, {
    "GET /api/v1/admin/projects": {
      status: 402,
      body: { code: "PRO_REQUIRED", message: "Pro required" },
    },
  });
  await page.goto("/admin/projects");
  await expect(page.getByText(/多项目工作区.*Pro/)).toBeVisible();
  await expect(
    page.getByRole("link", { name: /咨询 Pro 授权|Ask about Pro access/ }),
  ).toBeVisible();
...
async function trendBucketLabel(
  page: Page,
  bucket: number,
  range: "1h" | "30d",
) {
...
async function expectTrendTooltipLabel(
  page: Page,
  chart: Locator,
  expected: string,
  unexpected: string,
  ...
  await expect(label).toContainText(expected);
  await expect(label).not.toHaveText(unexpected);
}

test("Dashboard trends keep the previous chart while an uncached range loads", async ({
  page,
}) => {
  ...
  const ranges = page.getByRole("group", { name: /统计周期|Period/ });
  const nextRange = ranges.getByRole("button", { name: /30 天|30d/i });
  const oneHourLabel = await trendBucketLabel(
    page,
    initialPoints[0].bucket,
    "1h",
  );
  const thirtyDayLabel = await trendBucketLabel(
    page,
    initialPoints[0].bucket,
    "30d",
  );
...
test("Dashboard trends keep the previous chart and warn when an uncached range fails", async ({
  page,
}) => {
  const initialPoints = trendPoints(4, 12, 10);
  const requestStarted = deferred<void>();
  ...
  response.resolve({
    status: 500,
    body: { code: "FAILED", message: "fixture range failure" },
  });
  await expect(trends).not.toHaveAttribute("aria-busy", "true");
  await expect(chart).toBeVisible({ timeout: 20_000 });
  await expect(ranges).toBeVisible();
  await expect(nextRange).toHaveAttribute("aria-pressed", "true");
  await expect(trends).toContainText(/仍显示.*数据|retained.*data/i);
  await expect(chart.locator(".recharts-bar-rectangle").first()).toBeVisible();
  await expectTrendTooltipLabel(page, chart, oneHourLabel, thirtyDayLabel);
});
...
test("Dashboard trends poll at range-specific intervals", async ({ page }) => {
  ...
  await navigateClient(page, "/admin?range=1h");
  ...
  const oneHourRefetch = page.waitForResponse((response) => {
    ...
    return (
      url.pathname === "/api/v1/admin/dashboard/trends" &&
  ...
  await ranges.getByRole("button", { name: /24 小时|24h|24 hours/i }).click();
...
test("NowStrip keeps healthy cached data when its focus refetch fails", async ({
  page,
}) => {
  await pausePollingClock(page);
  let calls = 0;
    ...
    "GET /api/v1/now": () => {
      ...
      return calls === 1
        ? {
            status: "healthy",
            last_activity: null,
            rate: { requests_per_min: 7, ingress_bps: 10, egress_bps: 20 },
            upstreams: { healthy: 1, total: 1 },
          }
        : {
            status: 500,
            body: { code: "FAILED", message: "now refetch failed" },
          };
    },
  });
  await navigateClient(page, "/admin");
  ...
  await page.clock.runFor(100);
  await expect(now).toBeVisible();
  await expect(page.locator("[data-dashboard-overview]")).not.toContainText(
    /NaN|undefined/,
  );
});

test("Users stay visible when Tokens initially return 500", async ({
  page,
}) => {
  await mockAdminApi(page, {
    ...
    "GET /api/v1/admin/tokens": {
      status: 500,
      body: { code: "FAILED", message: "fixture tokens failure" },
    },
  });
...
test("Blocklist overrides stay visible when status initially returns 500", async ({
  page,
}) => {
  await mockAdminApi(page, {
    "GET /api/v1/admin/blocklist/status": {
      status: 500,
      body: { code: "FAILED", message: "fixture status failure" },
    },
    "GET /api/v1/admin/blocklist/overrides": {
      items: [populatedOverride],
      now: "2026-07-10T00:00:00Z",
    },
  });
  await page.goto("/admin/quarantine");
  await page
    .getByRole("combobox", { name: /拦截视图|Blocking view/ })
    .selectOption("blocklist");
  await expect(
    page.getByRole("table").getByText("false-positive"),
  ).toBeVisible();
  await expect(page.getByRole("alert")).toContainText("fixture status failure");
});

test("Dashboard snapshot failure keeps independent live and analytics regions mounted", async ({
  page,
}) => {
  await mockAdminApi(page, {
  ...
  await expect(
    page
      .getByRole("alert")
      .filter({ hasText: "fixture dashboard snapshot failure" }),
  ).toBeVisible();
  await expect(page.locator('[data-query-key="now"]')).toBeVisible();
  await expect(
    page.getByRole("heading", { name: /最近请求|Recent requests/ }),
  ).toBeVisible();
  await expect(
    page.locator('[data-query-key="dashboard-trends"]'),
  ).toBeVisible();
  await expect(
    page.getByRole("heading", { name: /性能快照|Performance snapshot/ }),
  ).toHaveCount(0);
});
...
test("Project package failure never renders the successful empty state", async ({
  page,
}) => {
  await mockAdminApi(page, {
    ...
    "GET /api/v1/admin/projects/1": {
      ...projectSummary,
      proxy_url: "https://example.test/p/fixture-project",
      ecosystem_breakdown: { pypi: 1 },
    },
    "GET /api/v1/admin/projects/1/packages": {
      status: 500,
      body: { code: "FAILED", message: "fixture packages failure" },
    },
  });
  await page.goto("/admin/projects");
  await page
    .getByRole("button", { name: /查看 fixture-project|View fixture-project/ })
    .click();
  await expect(
    page.getByRole("alert").filter({ hasText: "fixture packages failure" }),
  ).toBeVisible();
...
const mutationCases: MutationCase[] = [
  {
    ...
    endpoint: "POST /api/v1/admin/upstreams",
    status: 422,
    fixtures: {
  ...
  {
    name: "Quarantine approval revoke",
    path: "/admin/quarantine",
    endpoint: "DELETE /api/v1/admin/quarantine/approvals/1",
    status: 422,
    fixtures: {
    ...
    submit: async (page) => {
      ...
      await page
        .getByPlaceholder(/填写理由|Enter a reason/)
        .fill("fixture revoke reason");
      await page
        .getByRole("dialog")
    ...
    retained: (page) =>
      page.getByRole("dialog", { name: /撤销此放行|Revoke this approval/ }),
  },
];

for (const mutation of mutationCases) {
  test(`${mutation.name} retains context and never reports success after failure`, async ({
    page,
  }) => {
    ...
    await mockAdminApi(page, {
      ...
      [mutation.endpoint]: {
        ...
        body: { code: "MUTATION_FAILED", message },
      },
    });
...
test("Cache cleanup stays busy until success then closes and toasts the service message", async ({
  page,
}) => {
  ...
  const confirm = page
    .getByRole("dialog")
    .getByRole("button", { name: /确认清理|Confirm/ });
  await confirm.click();
  const cleaning = page
    .getByRole("dialog")
    .getByRole("button", { name: /清理中|Cleaning/ });
  await expect(cleaning).toHaveAttribute("aria-busy", "true");
  await expect(cleaning).toBeDisabled();
  cleanup.resolve();
  await expect(
    page.getByRole("dialog", { name: /清理缓存|Clean Cache/ }),
  ).toHaveCount(0);
  await expect(page.locator('[data-toast-tone="success"]')).toContainText(
    "fixture cleanup completed",
  );
});

test("Cache cleanup keeps a partial result visible instead of claiming success", async ({
  page,
}) => {
  await mockAdminApi(page, {
    ...
    "POST /api/v1/admin/cache/cleanup": {
      message: "fixture cleanup partially completed",
      outcome: "partial",
      deleted: 1,
      skipped: 1,
      failed: 1,
  ...
  await expect(
    dialog.getByRole("button", { name: /确认清理|Confirm/ }),
  ).toBeDisabled();
  await expect(page.locator('[data-toast-tone="success"]')).toHaveCount(0);
});

test("Cache delete clears an old failure before the confirmation dialog is reopened", async ({
  page,
}) => {
  const entry = {
    id: 41,
    key: "pypi/simple/fixture/index.html",
    adapter_type: "pypi",
  ...
  await mockAdminApi(page, {
    ...
    "DELETE /api/v1/admin/cache/41": {
      ...
      body: {
        ...
        message: "fixture delete failure",
      },
    },
  });
  await page.goto("/admin/cache");
  ...
  const openDelete = page.getByRole("button", {
    name: /删除缓存条目 pypi\/simple\/fixture\/index\.html|Delete cache entry pypi\/simple\/fixture\/index\.html/,
  });
...
test("Rule create recovers the list from an initial query failure with the returned entity", async ({
  page,
}) => {
  const createdRule = {
    id: 10,
    ecosystem: "npm",
    package_name: "recovered-rule",
    version: "*",
    action: "deny",
  ...
  await mockAdminApi(page, {
    "GET /api/v1/admin/rules": {
      status: 500,
      body: { code: "FAILED", message: "fixture rules unavailable" },
    },
    "POST /api/v1/admin/rules": createdRule,
  });
  await page.goto("/admin/rules");
  await expect(page.getByRole("alert")).toContainText(
    "fixture rules unavailable",
  );
...
test("Blocklist sync exposes a stable held pending state", async ({ page }) => {
  ...
  sync.resolve();
  await expect(
    page.getByRole("button", { name: /立即同步|Sync now/ }),
  ).not.toHaveAttribute("aria-busy", "true");
});

test("Security legacy-policy disable saves retain per-ecosystem busy, error, and normalized response state", async ({
  page,
}) => {
  ...
  await mockAdminApi(page, {
    ...
    "PUT /api/v1/admin/security/policies/cargo": async (request: Request) => {
      ...
      if (cargoCalls === 1) {
        ...
        return {
          status: 422,
          body: { code: "INVALID_POLICY", message: "fixture cargo rejected" },
        };
      }
  ...
  await page
    .getByRole("combobox", { name: /情报视图|Intelligence view/ })
  ...
  const pypiSave = pypiRow.getByRole("button", {
    name: /PYPI.*保存|PYPI.*Save/,
  });
  const cargoSave = cargoRow.getByRole("button", {
    name: /CARGO.*保存|CARGO.*Save/,
  });
  ...
  await expect(cargoRow.getByRole("alert")).toContainText(
    "fixture cargo rejected",
  );
