...
test('dashboard shows a compact live download snapshot and polls for new items', async ({ page }) => {
  ...
  await expect(feed.locator('[data-download-id]')).toHaveCount(2)
  expect(await feed.locator('[data-live-flow]').evaluate(element => getComputedStyle(element, '::after').animationName)).toBe('none')
  ...
  expect(await feed.locator('[data-live-flow]').evaluate(element => getComputedStyle(element, '::after').animationName)).toBe('liveDownloadSweep')
})
...
test('live download motion is explicit and respects reduced-motion on mobile', async ({ page }) => {
  ...
  expect(await feed.locator('[data-download-id]').first().evaluate(element => getComputedStyle(element).animationName)).toBe('none')
  expect(await feed.locator('[data-live-pulse]').evaluate(element => getComputedStyle(element).animationName)).toBe('none')
  expect(await feed.locator('[data-live-flow]').evaluate(element => getComputedStyle(element, '::after').animationName)).toBe('none')
})
