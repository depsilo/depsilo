...
const workspaceNavigation = [
  { id: "overview", label: "Overview", href: "/admin", routes: ["/admin"] },
  {
    ...
    routes: ["/admin/upstreams"],
  },
  {
    id: "cache",
    label: "Cache",
    href: "/admin/cache",
    routes: ["/admin/cache", "/admin/indexes", "/admin/compile-cache"],
  },
  {
    id: "logs",
    label: "Logs",
    href: "/admin/logs",
    routes: ["/admin/logs", "/admin/upstream-updates", "/admin/audit"],
  },
  {
    id: "security",
    label: "Security",
    href: "/admin/security",
    routes: ["/admin/security", "/admin/quarantine", "/admin/rules"],
  },
  {
    id: "projects",
    label: "Projects",
...
test("pending principal check shows an accessible branded loading state", async ({
  page,
}) => {
  ...
  await expect(
    pending.getByRole("status").locator("[data-brand-mark]"),
  ).toBeVisible();
...
test("closed mobile drawer has no focusable offscreen links", async ({
  page,
}) => {
  const longVersion = "0.2.0-126-g43ca7fe-dirty";
  await mockAdminApi(page, {
    "GET /api/v1/stats": {
      service: { version: longVersion, status: "healthy" },
      week: {},
      upstreams: [],
    },
  });
  ...
  const [versionBox, closeBox] = boxes as [
...
test("desktop navigation shows only workspaces and keeps destinations in page tabs", async ({
  page,
}) => {
  await mockAdminApi(page);
  ...
  expect(
    await groups.evaluateAll((elements) =>
      elements.map((element) => element.getAttribute("data-admin-nav-group")),
    ),
  ).toEqual(workspaceNavigation.map((workspace) => workspace.id));

  for (const workspace of workspaceNavigation) {
    const group = navigation.locator(
      `[data-admin-nav-group="${workspace.id}"]`,
    );
    const workspaceLink = group
      .locator("[data-admin-workspace-row]")
  ...
  for (const workspace of workspaceNavigation) {
    ...
    for (const href of workspace.routes) {
      ...
      await expect(tabs.locator('a[aria-current="page"]')).toHaveAttribute(
        "href",
        href,
      );
      await expect(
        group.locator('[data-admin-workspace-current="true"]'),
      ).toBeVisible();
      await expect(group.getByRole("link")).toHaveAttribute(
        "aria-current",
        href === workspace.href ? "page" : "location",
      );
      await expect(navigation.getByRole("link")).toHaveCount(6);
    }
  }
  ...
  const instanceLink = page
    .locator("[data-admin-sidebar-footer]")
  ...
  const instanceNavigation = page.locator(
    '[data-admin-page-navigation="instance"]',
  );
  ...
  expect([...visibleRouteHrefs].sort()).toEqual(
    legacyAdminHrefs
      .filter(
        (href) => href !== "/admin/attention" && href !== "/admin/bandwidth",
      )
      .sort(),
  );
  ...
  await expect(
    page
      .locator("main")
      .getByRole("heading", { level: 1, name: "Needs Attention" }),
  ).toBeVisible();
...
test("desktop sign-out control becomes visibly focused for keyboard users", async ({
  page,
}) => {
  await mockAdminApi(page);
  await page.setViewportSize({ width: 1440, height: 1000 });
...
test("mobile drawer selects a workspace and page tabs select its destination", async ({
  page,
}) => {
  await mockAdminApi(page);
  ...
  await expect(
    navigation.getByRole("link", { name: "总览", exact: true }),
  ).toBeFocused();
  ...
  const auditLogs = page
    .locator('[data-admin-page-navigation="logs"]')
  ...
  await trigger.click();
  await expect(
    navigation.getByRole("link", { name: "日志", exact: true }),
  ).toBeFocused();
  await expect(
    navigation.getByRole("link", { name: "日志", exact: true }),
...
test("desktop shell uses a 232px rail and separates breadcrumb from the page heading", async ({
  page,
}) => {
  await mockAdminApi(page);
  await setUiPreferences(page, "light", "en");
  ...
  await expect(
    page
      .locator("main")
      .getByRole("heading", { level: 1, name: "Metadata Refreshes" }),
  ).toHaveCount(1);
});

test("full live status stays on Dashboard and is removed from the topbar", async ({
  page,
}) => {
  await mockAdminApi(page);
  ...
  await expect(topbar).not.toContainText(
    /部分降级|性能下降|degraded|请求\/分钟|req\/min|出口|egress/i,
  );
...
test("desktop Admin chrome uses a clean canvas, brand portal link, and labeled theme control", async ({
  page,
}) => {
  await mockAdminApi(page);
  ...
  const brandLink = page
    .locator('[data-admin-nav-surface="sidebar"]')
    .locator("..")
    .getByRole("link", { name: "返回门户" });
  await expect(brandLink).toHaveAttribute("href", "/");
  await expect(topbar.getByRole("link", { name: "返回门户" })).toHaveCount(0);

  const preferences = page.locator("[data-admin-sidebar-preferences]");
  await expect(topbar.locator('[data-theme-toggle="labeled"]')).toHaveCount(0);
  await expect(topbar.locator('[data-language-toggle="admin"]')).toHaveCount(0);
  await expect(preferences).toBeVisible();
  await expect(
    preferences.locator('[data-language-toggle="admin"]'),
  ).toBeVisible();
  ...
  expect(await page.evaluate(() => localStorage.getItem("depsilo-theme"))).toBe(
    "dark",
  );
...
test("mobile Admin keeps the theme mode readable and exposes Portal through the drawer brand", async ({
  page,
}) => {
  await mockAdminApi(page);
  await setUiPreferences(page, "light", "en");
  ...
  const themeToggle = drawer.locator(
    '[data-admin-sidebar-preferences] [data-theme-toggle="labeled"]',
  );
...
test("failed now request never displays healthy", async ({ page }) => {
  await mockAdminApi(page, {
    "GET /api/v1/now": {
      status: 500,
      body: { code: "FAILED", message: "down" },
    },
  });
  await page.goto("/admin");
  const fullStatus = page.locator('main [data-query-key="now"]');
  await expect(
    fullStatus.getByText(
      /实时请求数据暂时不可用|Live request data is unavailable/,
    ),
  ).toBeVisible();
  await expect(fullStatus.getByText(/运行正常|已就绪/)).toHaveCount(0);
});

test("stale cached status keeps its refresh action visible", async ({
  page,
}) => {
  let calls = 0;
  await page.setViewportSize({ width: 390, height: 844 });
  ...
  await page.goto("/admin");
  const fullStatus = page.locator('main [data-query-key="now"]');
  const staleLabel = fullStatus
    .getByText("实时请求刷新失败，正在显示上次成功获取的数据。")
    .first();
  const refresh = page
    .locator("[data-dashboard-status]")
  ...
  const strip = page.locator("[data-dashboard-status]");
  expect(
    await refresh.evaluate(
      (button, root) => {
        ...
        return (
          buttonBox.right <= rootBox.right && buttonBox.left >= rootBox.left
        );
      },
...
test("principal failure gates the outlet until Retry succeeds", async ({
  page,
}) => {
  let calls = 0;
  await mockAdminApi(page, {
    "GET /api/v1/auth/me": () => {
      ...
      return calls === 1
        ? {
            status: 500,
            body: { code: "FAILED", message: "principal unavailable" },
          }
