...
func (h *Handler) handleRequest(c *gin.Context) {
	...
	result, err := h.cacheMgr.Get(c.Request.Context(), cacheKey, "cran", ttl, func(ctx context.Context) (io.ReadCloser, string, int64, string, error) {
	...
	adapter.LogAccess(c.Request.Context(), h.db, "cran", c.Request.Method, cacheKey, result.Hit, result.Upstream, time.Since(start), http.StatusOK, c.ClientIP(), written)
}
