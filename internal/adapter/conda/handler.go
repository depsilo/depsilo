...
func (h *Handler) handleRequest(c *gin.Context) {
	...
	result, err := h.cacheMgr.Get(c.Request.Context(), cacheKey, "conda", ttl, func(ctx context.Context) (io.ReadCloser, string, int64, error) {
	...
	adapter.LogAccess(h.db, "conda", c.Request.Method, cacheKey, result.Hit, "", time.Since(start), http.StatusOK, c.ClientIP(), written)
}
