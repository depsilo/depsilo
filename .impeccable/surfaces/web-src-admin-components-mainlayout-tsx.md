...
- **Job and outcome:** The first viewport answers whether the request path is healthy, why it is degraded, and what needs action. Six task workspaces replace seventeen equal-weight destinations: Overview, Upstreams, Cache, Logs, Security, and Projects, with instance administration in the sidebar footer.
- **Direction:** **Dependency Flowline / 依赖流线控制台**. The overview makes the real request path—client ingress → Depsilo cache → upstreams—the dominant composition, with one continuous signal axis and a compact inset attention rail beside it. The page heading and content share the full default fluid canvas, capped at 1840px, and one left baseline. The pure-white light canvas is separated from a neutral workspace rail (`#F6F7F5`, hover `#EAF0E9`); dark mode uses a matte charcoal-green canvas. Primary instruments remain open while supporting rails use surface contrast. Recent activity and one trend follow; full reports move into their expert workspaces.
...
| --- | --- | --- |
| Six-workspace navigation | Semantic links + footer instance link | Desktop shows six task workspaces; instance administration stays in the footer; the drawer uses the same order; old routes remain addressable |
